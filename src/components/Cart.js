...
const Cart = () => {
    ...
    const calculatePrice = () => {
        ...
        const price = cartItems.reduce((sum, item) => sum + item.count * (item?.defaultPrice ? item.defaultPrice / 100 : item.price / 100), 0);
                                    ...
                                    return (
                                        ...
                                        <div key={item.id} data-testid="cart-item-card" className="flex gap-4 justify-between items-center my-2 shadow-md bg-white">
                                            ...
                                            <div className="flex gap-1 pr-2">
                                                <span className="pr-1">₹{item?.price ? (item?.price === 0 ? item.defaultPrice / 100 : item.price / 100) : item.defaultPrice / 100}</span>
